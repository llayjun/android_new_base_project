...
public class GlideUtils {
    ...
    //默认加载
    public static void loadImageView(Context mContext, byte[] bytes, ImageView mImageView) {
        Glide.with(mContext).load(bytes).into(mImageView);
    }

    //默认加载
    public static void loadImageView(Context mContext, String path, ImageView mImageView, int error) {
        Glide.with(mContext).load(path).apply(new RequestOptions().error(error)).into(mImageView);
    }
    ...
    //加载指定大小
    public static void loadImageViewSize(Context mContext, Bitmap bitmap, int width, int height, ImageView mImageView) {
    ...
    //设置圆角图片
    public static void loadImageRoundCorner(Context mContext, int resId, ImageView mImageView, int roundingRadius) {
        Glide.with(mContext).load(resId)
                .apply(new RequestOptions().transforms(new CenterCrop(), new RoundedCorners(roundingRadius)))
                .into(mImageView);
    }
    ...
    //设置跳过磁盘和内存缓存
    public static void loadImageViewCacheAndDisk(Context mContext, Uri uri, ImageView mImageView) {
    ...
    //设置跳过磁盘和内存缓存
    public static void loadImageViewCacheAndDisk(Context mContext, Bitmap bitmap, ImageView mImageView) {
        Glide.with(mContext).load(bitmap)
                .apply(new RequestOptions().skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.NONE))
                .into(mImageView);
    }
    ...
    //设置圆形图片,设置跳过磁盘和内存缓存
    public static void loadImageViewCacheAndDiskRound(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path)
                .apply(new RequestOptions().skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.NONE))
                .apply(RequestOptions.circleCropTransform())
                .into(mImageView);
